...
limTau = zeros(6,samples);
limVel = zeros(6,samples);
limPosMax = zeros(6,samples);
limPosMin = zeros(6,samples);
for i=1:6
...
if plots(1)~=0
    ...
    plot_joints(t,1:6,qt,qDt,qDDt,FigNum,5,LineColor);
    plot_joints(t,1:6,limPosMax,limVel,qDDt,FigNum,5,['--',LineColor]);
    FigNum = plot_joints(t,1:6,limPosMin,-limVel,qDDt,FigNum,5,['--',LineColor]);
end
...
if plots(3)~=0
    ...
    LabelY = '$\tau$ ';
    plot_vector(t,1:6,Tau,FigNum,[4,4],LineColor,LabelX,LabelY);    
